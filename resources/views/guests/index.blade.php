...
{{-- <x-guest-layout>
        <form action="{{ route('confirmation.save', ['reservation_id' => $reservation->id]) }}" method="post">
            @csrf

            <div id="guests-container">
                <div class="guest">
                    <input type="text" name="guests[0][first_name]" placeholder="First Name">
                    <input type="text" name="guests[0][last_name]" placeholder="Last Name">
                    <input type="text" name="guests[0][cpf]" placeholder="CPF">
                    <input type="text" name="guests[0][age]" placeholder="Age">
                    <input type="hidden" name="guests[0][user_id]" value="{{ auth()->id() }}">
                    <input type="hidden" name="reservation_id" value="{{ $reservation->id }}">
                    <!-- Adicione os demais campos (CPF, idade, etc) para cada convidado -->
                </div>
            </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#add-guest').click(function() {
                    $('#guests-container').append(
                        '<div class="guest">' +
                        '<input type="text" name="guests[' + guestIndex +
                        '][first_name]" placeholder="First Name">' +
                        '<input type="text" name="guests[' + guestIndex +
                        '][last_name]" placeholder="Last Name">' +
                        '<input type="text" name="guests[' + guestIndex + '][cpf]" placeholder="CPF">' +
                        '<input type="text" name="guests[' + guestIndex + '][age]" placeholder="Age">' +
                        '<input type="hidden" name="guests[' + guestIndex +
                        '][user_id]" value="{{ auth()->id() }}">' +
                        '<input type="hidden" name="guests[' + guestIndex +
                        '][reservation_id]" value="{{ $reservation->id }}">' +
